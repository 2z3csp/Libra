...
* `--onedir` 配布では `--contents-directory _internal` を必須とし、非対応の PyInstaller ではビルドを中断してアップグレードを促す。
...
| 2026-02-10 | `--contents-directory` を onedir 配布の必須要件へ変更し、非対応版では明示エラーでビルド停止する方針へ改定 | `_internal` 集約を確実に担保し、依存ファイルが実行ファイル直下へ散在する状態を防ぐため | ビルド運用/互換性/配布品質 |
